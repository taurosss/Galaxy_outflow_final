...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb 25 19:31:13 2023
...
vel_map_N = np.full_like(flux_map_tmp, np.nan)
vdisp_map_N = np.full_like(flux_map_tmp, np.nan)
vdisp_map_N_test = np.full_like(flux_map_tmp, np.nan)
...
fit_params3g.add('cen_g1', value=0, min =-600, max = 600)
fit_params3g.add('wid_g1', value=100, min = 10, max = 500)
...
fit_params3g.add('wid_g2', expr = 'wid_g1', min = 10, max = 500)
...
fit_params3g.add('wid_g3', expr= 'wid_g1', min = 10, max = 500)


fit_params3gx = Parameters()
fit_params3gx.add('amp_g1', value=4000, min =150)                   #H_alpha
fit_params3gx.add('cen_g1', value=0,min =-600, max = 600)
fit_params3gx.add('wid_g1', value=100, min = 10, max = 500)
fit_params3gx.add('C_g3', value = 30, min = 0 )
fit_params3gx.add('amp_g2', value= 500, min =50)                    #N_II 6549
fit_params3gx.add('cen_g2', expr = 'cen_g1 + (6549.86 - 6564.61)/6564.61 * 299792.458 ')
# fit_params3gx.add('peak_split_1', value = -660, min = -720, max = -600)
# fit_params3gx.add('cen_g2', expr = 'cen_g1 + peak_split_1')
fit_params3gx.add('wid_g2', expr = 'wid_g1', min = 10, max = 500)
fit_params3gx.add('amp_g3', expr = 'amp_g2 * 3', min =50)             #N_II 6585
fit_params3gx.add('cen_g3', expr = 'cen_g1 +( 6585.27 - 6564.61)/ 6564.61 * 299792.458')
# fit_params3gx.add('peak_split_2', value = 930, min = 870, max = 990)
# fit_params3gx.add('cen_g3', expr = 'cen_g1 + peak_split_2')
fit_params3gx.add('wid_g3', expr= 'wid_g1', min = 10, max = 500)
...
# mod = fits.open(path + '/1.1mod.fits')[0].data
# mod_alpha_t = fits.open(path + '/2.1modalpha.fits')[0].data
# flux_map = fits.open(path + '/3.1flux_map_H.fits')[0].data
# vel_map = fits.open(path + '/4.1vel_H.fits')[0].data
# vdisp_map =fits.open(path + '/5.1disp_H.fits')[0].data
# flux_map_N = fits.open(path + '/9.1flux_map_N.fits')[0].data
# vel_map_N = fits.open(path + '/9.1vel_map_N.fits')[0].data
# vdisp_map_N = fits.open(path + '/9.1vdisp_map_N.fits')[0].data
# amperr =fits.open(path + '/6.fits')[0].data
# velerr = fits.open(path + '/7.fits')[0].data
# disperr = fits.open(path + '/8.fits')[0].data
# costerr = fits.open(path + '/9.fits')[0].data
...
for idxs in spiral_vect:
    ...
    ii = int(prt[2])    #x
    jj = int(prt[0])    #y
    # print('fit pixel:' +str(ii) + ' ' + str(jj))
    cont += 1
    spec_tmp = H_alpha[:,jj,ii]
    spec_tmp = np.nan_to_num(spec_tmp)
    # spec_tmp[0]=0
    # if all(nlargest(2, spec_tmp) > 10*error_alpha):      # thershold on the 2 biggest values of the spectra
    # if flux_map_tmp[jj,ii]>4.2*error:
    #     spec_tmp = datacube.data[:,jj,ii]
    #     spec_tmp = np.nan_to_num(spec_tmp)
    #     spec_tmp[0]=0
    if flux_map_tmp[jj,ii] > 7.5 * error_alpha:    
        # FIT WITH 2 GAUSSIAN
        if jj > ii :                  # imposing differnt limit on the pixel above the galaxy diagonal
            velmax = 100                             # limit to select the blueshifted pixel
            velmin = -600
        else:
            velmax = 600
            velmin=-100
        ...
        fit_params3g.add('cen_g1', value=out_1g[1], min = velmin, max = velmax)
        fit_params3g.add('wid_g1', value=out_1g[2], min = 10, max = 500)
        ...
        out = minimize(residual, fit_params3gx, args=(x,p,), kws={'data': spec_tmp,'sigma': error_alpha * 10})
        fitx, fit1x, fit1xz, fitx_N = residual(out.params, x, p)
        mod1x[:,jj,ii] = fitx
        mod_alphax[:, jj, ii] = fit1x
        parvals = out.params.valuesdict()
        amplitude_1 = parvals['amp_g1']
        stddev_1 = parvals['wid_g1']
        mean_1 = parvals['cen_g1']
        ...
        # cost1x_1err[jj,ii] = out.params['C_g3'].stderr
        out_1gx = [amplitude_1, mean_1, stddev_1, cost, amplitude_2, mean_2, stddev_2, amplitude_3, mean_3, stddev_3]
        bic_1gx = out.bic
        test_amp.append(amplitude_1)
        # chi1x = out.redchi
        
        # Pass the best fit parameters as initial parameters for the next pixel to fit
        # fit_params1gx.add('amp_g1', value=out_1gx[0], min = 100)
        # fit_params1gx.add('cen_g1', value=out_1g[1],)
        fit_params3gx.add('amp_g1', value=out_1gx[0], min =100)                   #H_alpha
        fit_params3gx.add('cen_g1', value=out_1gx[1],min = -550, max = 550)
        fit_params3gx.add('wid_g1', value=out_1gx[2], min = 10, max = 500)
        fit_params3gx.add('C_g3', value = out_1gx[3], min = 0 )
        fit_params3g.add('amp_g2', value= out_1gx[4], min =45)                    #N_II 6549
        fit_params3gx.add('cen_g2', expr = 'cen_g1 + (6549.86 - 6564.61)/6564.61 * 299792.458 ')
        # fit_params3gx.add('cen_g2', value = out_1gx[5], min = -3000, max = -400)
        fit_params3gx.add('wid_g2', value = out_1gx[6], min = 10, max = 500)
        fit_params3gx.add('amp_g3', value = out_1gx[7], min =40)             #N_II 6585
        # fit_params3g.add('cen_g3', value = out_1gx[8], min = -400, max = 8000)
        fit_params3gx.add('cen_g3', expr = 'cen_g1 + ( 6585.27 - 6564.61)/ 6564.61 * 299792.458')
        fit_params3gx.add('wid_g3', value = out_1gx[9], min = 10, max = 500)
        ...
        if printplot == True:
            ...
            plt.plot(x, spec_tmp, label='data')
            plt.plot(x, x*0,':',color = 'black')
            plt.plot(x, fitx, label='best fit')
            plt.plot(x, fit1x, label = 'alpha line')
            plt.xlabel('velocity [km/s]')
            plt.ylabel('flux [Jy]')
            plt.title('3gaussian_model_gx_pixel' + str(ii) + '-' + str(jj) )
            plt.legend()
            plt.show()
        if printfit == True:
            print('##')
            print('3gaussian_fit gx pixel:' + str(ii) + '-' + str(jj))
            report_fit(out)
        ...
        if jj in range1 and ii in range2:
            ...
            if bic_1g == bic_min:    
                ...
                vel_map_N[jj,ii] = np.nansum((fit_N*velocity_N)) * dlambda / flux_map_N[jj,ii]
                vdisp_map_N[jj,ii] = np.nansum((fit_N*(velocity_N-vel_map_N[jj,ii])**2)) * dlambda / flux_map_N[jj,ii]
                vdisp_map_N_test[jj,ii] = out_1g[2]
                ...
                vel_map_N[jj,ii] = np.nansum((fitx_N*velocity_N)) * dlambda / flux_map_N[jj,ii]
                vdisp_map_N[jj,ii] = np.nansum((fitx_N*(velocity_N-vel_map_N[jj,ii])**2)) * dlambda / flux_map_N[jj,ii]
                vdisp_map_N_test[jj,ii] = out_1gx[2]
...
plt.title('Velocity Dispersion')
plt.suptitle('Moments Maps H_alpha')
...
plt.title('Flux')
plt.subplot(132)
plt.imshow(vel_map_N, origin = 'lower', vmin = -500, vmax = 500, cmap ='jet')
plt.colorbar(shrink = 0.7, label='Velocity [km/s]')
plt.title('Velocity')
plt.subplot(133)
plt.imshow(vdisp_map_N, origin = 'lower',  vmin = 0, vmax=600, cmap = 'jet')
plt.colorbar(shrink = 0.7, label = 'Velocity Dispersion [km/s]')
plt.title('Velocity Dispersion')
plt.suptitle('Moments Maps N_II 6585')
# plt.figure(figsize = (12,4))
# plt.imshow(flux_map_N, origin = 'lower', cmap = 'jet')
...
hdul.writeto('1.2mod.fits', overwrite = True)
hdu = fits.PrimaryHDU(mod_alpha_t)
hdul = fits.HDUList([hdu])
hdul.writeto('2.2modalpha.fits', overwrite = True)
hdu = fits.PrimaryHDU(flux_map)
hdul = fits.HDUList([hdu])
hdul.writeto('3.2flux_map_H.fits', overwrite = True)
hdu = fits.PrimaryHDU(vel_map)
hdul = fits.HDUList([hdu])
hdul.writeto('4.2vel_H.fits', overwrite = True)
hdu = fits.PrimaryHDU(vdisp_map)
hdul = fits.HDUList([hdu])
hdul.writeto('5.2disp_H.fits', overwrite = True)

# hdu = fits.PrimaryHDU(amperr)
# hdul = fits.HDUList([hdu])
# hdul.writeto('6.1amperr_H.fits', overwrite = True)
# hdu = fits.PrimaryHDU(velerr)
# hdul = fits.HDUList([hdu])
# hdul.writeto('7.1velerr_H.fits', overwrite = True)
# hdu = fits.PrimaryHDU(disperr)
# hdul = fits.HDUList([hdu])
# hdul.writeto('8.1disperr_H.fits', overwrite = True)
hdu = fits.PrimaryHDU(flux_map_N)
hdul = fits.HDUList([hdu])
hdul.writeto('9.2flux_map_N.fits', overwrite = True)
hdu = fits.PrimaryHDU(vel_map_N)
hdul = fits.HDUList([hdu])
hdul.writeto('9.2vel_map_N.fits', overwrite = True)
hdu = fits.PrimaryHDU(vdisp_map_N)
hdul = fits.HDUList([hdu])
hdul.writeto('9.2vdisp_map_N.fits', overwrite = True)
hdu = fits.PrimaryHDU(vdisp_map_N_test)
hdul = fits.HDUList([hdu])
hdul.writeto('9.2vdisp_map_N_test.fits', overwrite = True)
# #----------------------------------------------------------------------------
