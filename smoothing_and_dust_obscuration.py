...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 29 18:29:54 2023
...
mod_alpha_t = fits.open(path + '/moments_map/NGC6810_MUSE/H_alpha/model_alpha_6_3gx.fits')[0].data
...
flux_NII = fits.open(path + '/9.1flux_map_N.fits')[0].data
...
# C_alpha = np.median(mod_alpha_t, axis=0)
# flux_H_alpha = 1 * flux_H_alpha - C_alpha # removing the coontinuum forgotten in the creation of moment map of H_alpha 
# for jj in range(40,277):
#     for ii in range(41,50):
#         flux_H_alpha[jj,ii] = flux_NII[jj,ii] * 1.79
# for jj in range(40,278):
#     for ii in range(250,280):
#         flux_H_alpha[jj,ii] = flux_NII[jj,ii] * 1.79
        
# for jj in range(44,244):
#     flux_H_alpha[jj,245] = flux_NII[jj,245] * 1.79
    
# for jj in range(202,244):
#     flux_H_alpha[jj,74] = flux_NII[jj,74] * 1.79
    
# for ii in range(74,245):
#         flux_H_alpha[243,ii] = flux_NII[243,ii] * 1.79
        
# hdu = fits.PrimaryHDU(flux_H_alpha)
# hdul = fits.HDUList([hdu])
# hdul.writeto('flux_map_H_definitive.fits', overwrite = True)

# Plot            
plt.figure(figsize = (14,9))

plt.subplot(221)
plt.imshow(np.log(flux_H_alpha), origin = 'lower', cmap = 'jet', vmin = 0)
plt.colorbar(shrink = 0.7, label = 'log(Flux [Jy])')
plt.title('H_alpha')
plt.subplot(222)
plt.imshow(np.log(flux_NII), origin = 'lower', cmap ='jet')
...
# -------------------------------------------------------
# tot_flux_alpha = np.nansum(flux_H_alpha)
# tot_flux_beta = np.nansum(flux_H_beta)
tot_flux_alpha = np.nansum(flux_H_alpha[mask])
tot_flux_beta = np.nansum(flux_H_beta[mask])
a_frac_b = tot_flux_alpha / tot_flux_beta
E_b_a = 2.5 * np.log10(a_frac_b) - 2.5*np.log10(2.86)
E_B_V = E_b_a / 1.07
A_h_alpha = 2.45 * E_B_V 

flux_H_alpha_corrected = flux_H_alpha * 10**(A_h_alpha/2.5)
hdu = fits.PrimaryHDU(flux_H_alpha_corrected)
hdul = fits.HDUList([hdu])
hdul.writeto('flux_map_H_definitive_dust_corrected.fits', overwrite = True)
# a_frac_b = np.full(flux_H_alpha, np.nan)
# a_frac_b[mask] = flux_H_alpha[mask] / flux_H_beta[mask] 
# flux_H_alpha[np.isnan(flux_H_alpha)] = 0.0
# plt.imshow(flux_H_alpha)
# hdu = fits.PrimaryHDU(flux_H_alpha)
# hdul = fits.HDUList([hdu])
# hdul.writeto('3.2flux_map_H_notnan.fits', overwrite = True)
