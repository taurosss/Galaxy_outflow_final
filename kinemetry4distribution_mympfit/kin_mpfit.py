...
Perform Levenberg-Marquardt least-squares minimization, based on MINPACK-1.
 ...
 - Converted from Numeric to numpy (Sergey Koposov, July 2008)
    ...
    V1.2.3: Included optional `args` keyword.
        Redefined relative step of numerical Jacobian like least_squares
        h = relstep*np.maximum(1, abs(x)) instead of h = relstep*abs(x)
        MC, Oxford, 17 January 2020
...
class mpfit:
    ...
    def __init__(self, fcn, xall=None, functkw={}, parinfo=None,
                 ...
                 quiet=False, diag=None, epsfcn=None, args=()):
        """
  Inputs:
    fcn:
  ...
  Keywords:
     ...
     iterfunct:
        ...
        MPFIT routine.  It should be declared in the following way:
           def iterfunct(myfunct, p, iter, fnorm, functkw=None, args=None,
   ...
   Outputs:
     ...
     .perror
        ...
        *If* you can assume that the true reduced chi-squared value is unity --
        ...
        [self.status, fvec] = self.call(fcn, self.params, args, functkw)
        ...
        while(1):
            ...
            if (nprint > 0) and (iterfunct is not None):
                if ((self.niter - 1) % nprint) == 0:
                    ...
                    status = iterfunct(fcn, self.params, self.niter, self.fnorm**2,
                       functkw=functkw, parinfo=parinfo, quiet=quiet, dof=dof, args=args, **iterkw)
            ...
            fjac = self.fdjac2(fcn, x, fvec, step, qulim, ulim, dside,
                          ...
                          functkw=functkw, ifree=ifree, xall=self.params, args=args)
            if fjac is None:
            ...
            while(1):
                ...
                [self.status, wa4] = self.call(fcn, self.params, args, functkw)
        ...
        if (nprint > 0) and (self.status > 0):
            catch_msg = 'calling ' + str(fcn)
            [status, fvec] = self.call(fcn, self.params, args, functkw)
...
###############################################################################
    ...
    def defiter(self, fcn, x, iter, fnorm=None, functkw=None,
                       ...
                       format=None, pformat='%.10g', dof=1, args=None):

        if quiet:
            return
        if fnorm is None:
            [status, fvec] = self.call(fcn, x, args, functkw)
            fnorm = norm(fvec)**2
...
###############################################################################
    ...
    def call(self, fcn, x, args, functkw, fjac=None):

        if self.qanytied:
            x = self.tie(x, self.ptied)
        self.nfev += 1
        f = fcn(x, *args, fjac=fjac, **functkw)
...
###############################################################################
    ...
    def fdjac2(self, fcn, x, fvec, step=None, ulimited=None, ulimit=None,
               ...
               xall=None, ifree=None, dstep=None, args=None):
        ...
        if autoderivative == 0:
            ...
            self.call(fcn, xall, args, functkw, fjac=fjac)
        ...
        # if absolute step is given, use that
        # DSTEP includes the fixed parameters
        if step[0] is None:
            dstepi = dstep[ifree]
            wh = dstepi > 0
            if np.any(wh):
                h[wh] = dstepi[wh]*np.maximum(1.0, np.abs(x[wh]))  # as in least_squares()
        else:
            stepi = step[ifree]
            wh = stepi > 0
            if np.any(wh):
        ...
        for j in range(n):
            xp = xall.copy()
            xp[ifree[j]] += h[j]
            [status, fp] = self.call(fcn, xp, args, functkw)
            ...
            if -1 <= dside[ifree[j]] <= 1:
                ...
                [status, fm] = self.call(fcn, xp, args, unctkw)
                if status < 0:
                    return None
