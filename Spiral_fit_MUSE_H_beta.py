...
fit, fitz = residual(out.params, x, p)
print('##')
...
# flux_map[M0< 4.5 *error_beta] = np.nan
# vel_map[M0 < 4.5 *error_beta] = np.nan
# vdisp_map[M0 < 4.5 *error_beta] = np.nan                 

# flux_map[300:,:] = np.nan
# vel_map[300:, :] = np.nan
# vdisp_map[300:,:] = np.nan

# nn = 3
# flux_map = fits.open(path + '/flux_map_spiral_' + str(nn) +'_1gx' + 'NGC6810_MUSE_H_beta' + '.fits')[0].data
# vel_map = fits.open(path + '/vel_map_spiral_'+ str(nn) +'_1gx' + 'NGC6810_MUSE_H_beta' + '.fits')[0].data
# vdisp_map =fits.open(path + '/vdisp_map_spiral_'+ str(nn) +'_1gx'  + 'NGC6810_MUSE_H_beta' + '.fits')[0].data

flux = fits.open(path + '/moments_map/NGC6810_MUSE/H_beta/flux_map_spiral_4_1gxNGC6810_MUSE_H_beta_clean.fits')[0].data  #già moltiplicato per dv
vel = fits.open(path + '/moments_map/NGC6810_MUSE/H_beta/vel_map_spiral_4_1gxNGC6810_MUSE_H_beta_clean.fits')[0].data
disp = fits.open(path + '/moments_map/NGC6810_MUSE/H_beta/vdisp_map_spiral_4_1gxNGC6810_MUSE_H_beta_clean.fits')[0].data


plt.figure(figsize=(12,4))
plt.suptitle('H_beta Moments map')
plt.subplot(131)
plt.imshow(flux.data, origin = 'lower', cmap = 'jet')
plt.colorbar()
plt.subplot(132)
plt.imshow(vel.data, origin = 'lower', vmin = -400, vmax = 400, cmap ='jet')
plt.colorbar()
plt.subplot(133)
plt.imshow(disp.data, origin = 'lower',  vmin = 0, vmax =300, cmap = 'jet')
plt.colorbar()
...
# hdul.writeto('model_' + str(nn) + '_1gx' + 'NGC6810_MUSE_H_beta' +'.fits', overwrite = True)


# hdu = fits.PrimaryHDU(flux_map)
# hdul = fits.HDUList([hdu])
# hdul.writeto('flux_map_spiral_' + str(nn) +'_1gx' + 'NGC6810_MUSE_H_beta_clean' + '.fits', overwrite = True)
# hdu = fits.PrimaryHDU(vel_map)
# hdul = fits.HDUList([hdu])
# hdul.writeto('vel_map_spiral_'+ str(nn) +'_1gx' + 'NGC6810_MUSE_H_beta_clean' + '.fits', overwrite = True)
# hdu = fits.PrimaryHDU(vdisp_map)
# hdul = fits.HDUList([hdu])
# hdul.writeto('vdisp_map_spiral_'+ str(nn) +'_1gx'  + 'NGC6810_MUSE_H_beta_clean' + '.fits', overwrite = True)
